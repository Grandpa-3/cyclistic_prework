...
SELECT
  ...
  -- Adding 5 years to the start and stop time to simulate recent data
...
INNER JOIN
  `your_project.your_dataset.zip_codes` AS ZIPSTARTNAME
  ON ZIPSTART.zip_code = CAST(ZIPSTARTNAME.zip AS STRING)
INNER JOIN
  `your_project.your_dataset.zip_codes` AS ZIPENDNAME
...
GROUP BY
  ...
  DATE_ADD(DATE(TRI.starttime), INTERVAL 5 YEAR),
  DATE_ADD(DATE(TRI.stoptime), INTERVAL 5 YEAR),
  WEA.temp,
  WEA.wdsp,
  WEA.prcp,
